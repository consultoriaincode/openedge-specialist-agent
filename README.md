...
1. **Mandatory gap analysis**: Compare item by item ALL features of the original code (or requirements) with the generated/migrated code. Explicitly list each feature as: ? migrated, ?? not-migratable (with documented justification referencing framework limitation), or ?? preserved in legacy layer.
